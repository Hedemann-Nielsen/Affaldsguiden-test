...
export const CommentsCopy = ({ stationId }) => {
	const { supabase } = useSupabase();
	const { loginData } = useAuth();
	const [reviewsData, fetchReviews] = useReviewsData(stationId); // Tilføj fetchReviews for at genindlæse data
	...
	const handleSubmitReview = async () => {
		try {
			...
			if (error) {
				...
				clearMessages();
				console.log("Kommentar blev sendt", data);
